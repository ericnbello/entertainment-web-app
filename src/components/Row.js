...
function Row ({title, url}) {
    ...
    // const [{mediaList, setMediaList }, setState] = useState(initialState);
                    ...
                    return (                    
                        <div key={video.id} className="flex flex-col text-xl text-white col-span-1 justify-start w-48">
                            ...
                            <div className="flex flex-col flex-wrap py-2">
                                <p className="flex pt-1 text-sm font-light">{(video.first_air_date == null ? video.release_date : video.first_air_date).substring(0,4)} • 
                                    <span className="px-2 pt-1">
                                        <div className="flex">
                                            <img src={video.media_type == null ? '' : video.media_type == "movie" ? "/assets/icon-category-movie.svg" : "/assets/icon-category-tv.svg" } />
                                            <p>
